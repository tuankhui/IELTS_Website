...
router.post('/evaluate-task1', authenticateToken, async (req, res) => {
...
Task (Ielts writing task 1): ${tasks.task}
...
Question 2: BOLD ALL GRAMMAR MISTAKE. (ONLY GIVE THE PARAGRAPH DO NOT PRINT ANYTHING ELSE)
...
[Answer question 10]
        ...
        // const result = await evaluateCollection.insertOne({ task_id: task_id, paragraph, cntWord: ((paragraph.trim().split(/\s+/).filter(Boolean)).length).toString(), cntLinking, cntGramar:ans[0], boldmistake: ans[1], coherence: ans[2], des_coherenece:ans[3],  lexical:ans[4], des_lexical:ans[5], gramar_range: ans[6], des_gramar_range:ans[7], task_archive:ans[8], des_task_archive:ans[9], created_at: new Date(), generated_by: username, raw });
